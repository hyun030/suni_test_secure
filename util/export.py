...
"""
완전 통합 보고서 생성 모듈
- matplotlib 기반 차트 생성 -> PNG 버퍼 -> reportlab로 PDF 삽입
- 순서 고정:
  1. 재무분석 결과
    1-1 정리된 재무지표 (표)
    1-1-1 분기별 트렌드 차트
...
# Optional OpenAI integration (사용 안해도 됨)
try:
    import openai
    GPT_AVAILABLE = True
except Exception:
...
# -------------------------
# 사용자 환경에 맞춰 폰트 경로 수정하세요
# -------------------------
def get_font_paths():
    """
    프로젝트 내 fonts/ 폴더에 한글 TTF를 넣어두고 경로를 지정하면
    그래프/리포트의 한글이 더 잘 보입니다.
    """
    return {
...
def register_fonts_safe():
    """
    reportlab에 폰트를 등록하고, 사용 가능한 폰트 이름을 dict로 리턴.
    실패 시 기본 라틴 폰트로 fallback.
    """
...
# matplotlib 한글 폰트 자동 설정 (가능하면)
def setup_matplotlib_fonts():
    try:
        font_paths = get_font_paths()
        # 우선 NanumGothic 사용 시도
        path = font_paths.get("Korean")
...
def clean_ai_text(raw):
    ...
    blocks = []
    for ln in lines:
        if ln.startswith('#') or ln[:2].isdigit() or ln.lower().startswith('##'):
...
def compute_gap_analysis(financial_df, sk_name_keyword='SK'):
    """
    financial_df: DataFrame with '구분' column and company columns
    returns: (gap_df (wide, for table), gap_chart_df (long, for chart))
    """
    try:
...
# -------------------------
# 차트 생성 (matplotlib) — 반드시 Figure 반환
# -------------------------
    ...
    try:
        ...
        try:
            ...
            for comp in gap_chart_df['회사'].unique():
                ...
                ax.bar(sub['지표'], sub['갭(%)'], label=comp, alpha=0.8)
            ax.axhline(0, color='red', linewidth=1)
            ax.set_title(title)
            ax.legend()
            fig.tight_layout()
...
# 재무 섹션 통합 (정해진 순서)
# -------------------------
def add_financial_section_full(story, financial_df, quarterly_df, registered_fonts, HEADING_STYLE, BODY_STYLE, insights=None):
    """
    Adds:
      1-1 정리표
      1-1-1 분기별 트렌드 차트
      1-2 갭차이 표
      1-2-1 갭차이 차트
      1-3 AI 재무 인사이트 (insights arg used)
    """
    # 1-1
    ...
    if insights:
        ...
        for typ, line in blocks:
            if '|' in line:
                # treat ASCII table
                ascii_lines = [l for l in insights.splitlines() if '|' in l]
                tbl = ascii_to_table(ascii_lines, registered_fonts, header_color='#E31E24')
                if tbl:
                    story.append(tbl)
                    story.append(Spacer(1,8))
                break
...
def create_enhanced_pdf_report(
    ...
    add_financial_section_full(story, financial_data, quarterly_df, registered_fonts, HEADING_STYLE, BODY_STYLE, insights=insights)

    # AI 인사이트 (자세)
    # 이미 1-3에 기본 설명 들어가지만, add_ai_insights_section를 통해 상세 출력
    story.append(Paragraph("AI 상세 인사이트", HEADING_STYLE))
    add_chunked = False
    add_ai_insights_section = None  # avoid name collision below
    # call directly:
    try:
        # reuse add_ai_insights_section implementation above
        # We'll create a small wrapper (function defined earlier) - but we didn't; reuse logic inline:
        blocks = clean_ai_text(insights)
        if not blocks:
            story.append(Paragraph("AI 인사이트가 제공되지 않았습니다.", BODY_STYLE))
            story.append(Spacer(1,12))
            ...
            for typ, line in blocks:
                if '|' in line:
                    ascii_lines = [l for l in str(insights).splitlines() if '|' in l]
                    tbl = ascii_to_table(ascii_lines, registered_fonts, header_color='#E31E24')
                    if tbl:
                        story.append(tbl)
                        story.append(Spacer(1,8))
        ...
        story.append(Paragraph("AI 인사이트 출력 중 오류가 발생했습니다.", BODY_STYLE))
        story.append(Spacer(1,12))
...
def generate_report_with_gpt_insights(
    ...
    excel_bytes = create_excel_report(financial_data, news_data, insights)
    return {'pdf': pdf_bytes, 'excel': excel_bytes}
...
if __name__ == "__main__":
    ...
    df_news = pd.DataFrame({
        '제목': ['뉴스1 제목', '뉴스2 제목', '뉴스3 제목'],
        '내용': ['뉴스1 내용 전문...', '뉴스2 내용 전문...', '뉴스3 내용 전문...']
    })

    # AI 인사이트 예시 (ASCII 테이블 포함)
    ...
    print("완성: final_report.pdf / final_report.xlsx")
