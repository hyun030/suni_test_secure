...
아래 6개 섹션을 **HTML만**으로 출력하세요. (중요: 코드블록 금지, 마크다운 금지)
각 섹션은 이 카드 템플릿을 사용하세요.

<!-- 카드 템플릿 -->
<div style="margin:16px 0;padding:16px 18px;border:1px solid #e9ecef;border-radius:12px;background:#ffffff;box-shadow:0 4px 12px rgba(0,0,0,.05)">
  <div style="font-weight:800;font-size:18px;display:flex;gap:10px;align-items:center;margin-bottom:8px">
    <span style="font-size:20px">[이모지]</span>
    <h3 style="margin:0;font-size:18px">[섹션 제목]</h3>
  </div>
  <ul style="margin:0 0 0 20px;padding:0;line-height:1.6">
    <li>…짧은 문장…</li>
    <li>…숫자/격차/추세는 괄호로 요약…</li>
  </ul>
</div>

[섹션 목록]
1) [이모지=📊][제목=1. SK에너지 현재 재무 상황 분석]
- 최근 수익성 변화와 원인 진단
- 경쟁사(GS칼텍스 등) 대비 수익구조의 강점/약점
- 영업이익률, 순이익률, 원가율, 판관비율 등 주요 지표 비교

2) [🔍] 2. 경쟁사 대비 사업 경쟁력 분석
- 경쟁사 대비 원가 효율성, 수익성, 비용 구조 차이
- SK에너지가 개선하거나 강화할 수 있는 포인트 도출


3) [🧩] 3. 전략적 시사점 및 내부 개선 방향
...
5) [📐] 5. DART 공시 데이터를 바탕으로 SK에너지와 경쟁사 간 갭 분석 및 추세 기반 전략 도출
[입력 데이터]
...
[결과 양식]
아래 마크다운 템플릿을 그대로 출력해주세요.
행을 합치지 말아주세요
각 항목은 반드시 개행(새 줄) 로 구분하고, 쉼표/점/중간점으로 합치지 말아주세요.
📊 경쟁사 비교 분석
...
[출력 규칙]
- 위 카드 템플릿을 그대로 사용하되, [이모지]/[섹션 제목]을 실제 값으로 대체
- 목록은 <ul><li>…</li></ul> 만 사용 (마크다운 불가)
- 섹션 사이 구분선(---) 출력 금지
"""
