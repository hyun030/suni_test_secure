...
당신은 SK에너지의 수석 전략 분석가입니다. 아래 **경쟁사 최신 뉴스 목록**을 종합하여
요약이 아닌 **교차·통합 인사이트**를 도출하고, 벤치마킹 아이디어와 실행계획까지 제시하세요.
모든 출력은 한국어이며, 표는 **마크다운 표**로 작성하세요. (행/열 합치기 금지, 각 불릿은 반드시 새 줄에서 시작)
...
## 1. 🧭 통합 분석 요약 (3~5문장)
- 여러 기사에 공통으로 드러난 핵심 트렌드 2~3개
- SK에너지 관점의 즉시 함의(원가/수익/리스크 관점 각 1문장 이상)
...
## 3. 🧭 경쟁지형 한눈에 보기(Table)
| 회사 | 강점 키워드 | 약점 키워드 | 최근 핵심 액션(1문장) | SK 시사점(1문장) |
|---|---|---|---|---|
| S-Oil |  |  |  |  |
| GS칼텍스 |  |  |  |  |
| HD현대오일뱅크 |  |  |  |  |
...
## 4. ⚠️ 리스크 & 워치리스트
- 위험 Top 3 (각 1문장): [이슈] → [영향 경로] → [시그널/임계값]
- 감시 항목 표:
| 항목 | 트리거/임계값 | 가능성 | 영향도 | 모니터링 지표 | 다음 점검일 |
|---|---|---|---|---|---|
| 예: 정제마진 급락 | < X $/bbl 2분기 지속 | 중 | 높음 | 마진/스프레드 | YYYY-MM-DD |

---

## 5. 🚀 90일 액션 플랜(Table)
| 과제 | 담당(가칭) | 마일스톤/기한 | KPI(수치 목표) | 예상 효과(정량/범위) |
|---|---|---|---|---|
| 예: 동력비 최적화 파일럿 | 생산기술팀 | W+6, W+12 | 동력비 -x% | 원가 △~△ |

---

[지침]
- 기사 본문이 없거나 수치가 명확하지 않으면 과도한 단정 금지, **범위** 또는 **정성 추정**으로 표기
- 숫자 단위 명시(%, $/bbl 등), 불릿은 **각 항목마다 새 줄**, 항목 사이 **빈 줄 1개**
- 가능하면 아이디어의 재무효과는 보수적 범위로 제시
"""
