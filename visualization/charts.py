...
def create_cost_analysis_chart(quarterly_df: pd.DataFrame, company_name: str = None):
    """고정비 vs 변동비 분석 차트"""
    if not PLOTLY_AVAILABLE or quarterly_df.empty:
        return None
    
    # 고정비, 변동비 관련 컬럼 찾기
    fixed_cost_cols = [col for col in quarterly_df.columns if any(keyword in col for keyword in ['고정비', '감가상각비', '인건비', '임차료', '관리비'])]
    variable_cost_cols = [col for col in quarterly_df.columns if any(keyword in col for keyword in ['변동비', '판매수수료', '운반배송비', '포장비', '외주가공비', '판촉비', '샘플비', '소모품비', '동력비', '원재료비'])]
    
    if not fixed_cost_cols and not variable_cost_cols:
        return None
    ...
    for _, row in quarterly_df.iterrows():
        ...
        # 고정비 합계
        fixed_total = sum(float(row.get(col, 0) or 0) for col in fixed_cost_cols)
        if fixed_total > 0:
            chart_data.append({
                '분기': quarter,
                '비용유형': '고정비',
                '금액': fixed_total,
                '회사': row.get('회사', company_name or 'Unknown')
            })
        
        # 변동비 합계
        variable_total = sum(float(row.get(col, 0) or 0) for col in variable_cost_cols)
        if variable_total > 0:
            chart_data.append({
                '분기': quarter,
                '비용유형': '변동비',
    ...
    fig = px.bar(
        df, x='분기', y='금액', color='비용유형',
        title="🏢 고정비 vs 변동비 분석",
        barmode='group',
        color_discrete_map={'고정비': '#ff7f0e', '변동비': '#2ca02c'},
        height=400
    )
...
def create_contribution_margin_chart(quarterly_df: pd.DataFrame, company_name: str = None):
    """공헌이익 분석 차트"""
    if not PLOTLY_AVAILABLE or quarterly_df.empty:
        return None
    
    # 공헌이익 관련 컬럼 찾기
    contribution_cols = [col for col in quarterly_df.columns if '공헌이익' in col]
    revenue_cols = [col for col in quarterly_df.columns if '매출액' in col]
    ...
    for _, row in quarterly_df.iterrows():
        quarter = row.get('분기', '')
        company = row.get('회사', company_name or 'Unknown')
    ...
    fig = px.line(
        df, x='분기', y='금액', color='지표', line_group='회사',
        title="💰 공헌이익 vs 매출액 추이",
        color_discrete_map={'공헌이익': '#d62728', '매출액': '#1f77b4'},
        height=400
    )
...
def create_cost_structure_pie_chart(quarterly_df: pd.DataFrame, quarter: str = None, company_name: str = None):
    """비용 구조 파이 차트"""
    if not PLOTLY_AVAILABLE or quarterly_df.empty:
        return None
    ...
    # 특정 분기 데이터 필터링
    if quarter:
        df_filtered = quarterly_df[quarterly_df['분기'] == quarter]
    else:
        # 가장 최근 분기 사용
        df_filtered = quarterly_df.iloc[-1:].copy()
    ...
    for item in fixed_items:
        for col in quarterly_df.columns:
            if item in col:
                value = float(row.get(col, 0) or 0)
                if value > 0:
                    cost_items[f'고정비-{item}'] = value
                    fixed_total += value
    
    # 변동비 항목들
    variable_items = ['판매수수료', '운반배송비', '포장비', '외주가공비', '판촉비', '샘플비', '소모품비', '동력비', '원재료비']
    variable_total = 0
    ...
    for label in labels:
        if '고정비' in label:
            colors.append('#ff7f0e')  # 주황색
        else:
            colors.append('#2ca02c')  # 초록색
    ...
    quarter_display = quarter or df_filtered['분기'].iloc[0]
    fig.update_layout(
        title=f"🏢 {company} 비용 구조 분석 ({quarter_display})",
        font=dict(family="Malgun Gothic, Apple SD Gothic Neo, sans-serif"),
        height=500
    )
...
def create_cost_trend_analysis(quarterly_df: pd.DataFrame, company_name: str = None):
    """비용 추세 분석 차트"""
    if not PLOTLY_AVAILABLE or quarterly_df.empty:
        return None
    ...
    cost_columns = []
    for col in quarterly_df.columns:
        if any(keyword in col for keyword in ['감가상각비', '인건비', '임차료', '관리비', '판매수수료', '운반배송비', '포장비', '외주가공비', '판촉비', '샘플비', '소모품비', '동력비', '원재료비']):
    ...
    for _, row in quarterly_df.iterrows():
        ...
        for col in cost_columns:
            ...
            if value > 0:
                # 비용 유형 분류
                if any(keyword in col for keyword in ['감가상각비', '인건비', '임차료', '관리비']):
                    cost_type = '고정비'
                else:
                    cost_type = '변동비'
    ...
    fig = px.scatter(
        df, x='분기', y='금액', color='비용유형', size='금액',
        title="📈 비용 추세 분석",
        color_discrete_map={'고정비': '#ff7f0e', '변동비': '#2ca02c'},
        height=400
    )
