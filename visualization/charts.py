...
def create_flexible_trend_chart(quarterly_df: pd.DataFrame, selected_metrics: list = None):
    """사용자가 선택한 지표들로 멀티라인 트렌드 차트 생성 (사진 스타일)"""
    ...
    # Y축이 여러 개인지 확인 (단위가 다른 지표들)
    percentage_metrics = [m for m in selected_metrics if '(%)' in m]
    amount_metrics = [m for m in selected_metrics if '(조원)' in m or '(억원)' in m]
    
    for company in companies:
        company_data = quarterly_df[quarterly_df['회사'] == company]
        base_color = get_company_color(company, companies)
        ...
        for i, metric in enumerate(selected_metrics):
            if metric in company_data.columns:
                ...
                # Y축 결정 (비율과 금액을 분리)
                yaxis = 'y2' if metric in amount_metrics and percentage_metrics else 'y'
    ...
    # 이중 Y축 설정 (비율과 금액이 섞여있는 경우)
    if percentage_metrics and amount_metrics:
        layout_kwargs.update({
            'yaxis': dict(
                title=' / '.join([m.replace('(%)', '(%)') for m in percentage_metrics]),
                side='left'
            ),
            'yaxis2': dict(
                title=' / '.join([m.replace('(조원)', '(조원)').replace('(억원)', '(억원)') for m in amount_metrics]),
                side='right',
                overlaying='y'
            )
        })
        ...
        # 단일 Y축
        unit = '(%)' if percentage_metrics else '(조원)' if any('조원' in m for m in selected_metrics) else ''
        layout_kwargs['yaxis_title'] = ' / '.join([m.replace(unit, '') for m in selected_metrics]) + unit
