...
class FinancialDataProcessor:
    ...
    CONCEPT_REGEX = {
        ...
        # DART API 호환 매핑 (계정과목 → 표준 항목)
    INCOME_STATEMENT_MAP = {
        # 매출
        'revenue': '매출액', 'sales': '매출액', '매출': '매출액', '매출액': '매출액',
        '제품매출': '매출액', '상품매출': '매출액',
        # 매출원가
        'costofgoodssold': '매출원가', 'cogs': '매출원가', 'costofsales': '매출원가',
        '매출원가': '매출원가', '원가': '매출원가', '상품매출원가': '매출원가',
        # 매출총이익
    ...
    def process_dart_data(self, dart_df: pd.DataFrame, company_name: str) -> pd.DataFrame | None:
        """
        DART API 결과(DataFrame; 보통 account_nm, thstrm_amount 컬럼)를
        표준 손익계산서 DataFrame으로 변환 (main_app 호환용)
        """
