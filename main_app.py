    ...
    if SessionManager.is_data_available('quarterly_data'):
        ...
        if PLOTLY_AVAILABLE:
            ...
            if available_metrics:
                ...
                # 필터링된 데이터 생성
                filtered_data = manual_chart_data.copy()
                ...
                if (bar_metrics or line_metrics) and not filtered_data.empty:
                    ...
                    if flexible_chart:
                        ...
                        st.plotly_chart(flexible_chart, use_container_width=True, key="manual_flexible_trend")
                        
                        # 선택된 설정 요약
                        st.success(f"✅ 현재 표시 중: 회사 {len(selected_companies_chart)}개, 총 지표 {total_metrics}개")
                ...
                st.warning("사용 가능한 지표가 없습니다. 수동 업로드 데이터를 다시 확인해주세요.")
        else:
            st.info("📊 차트 모듈이 없거나 차트용 데이터 변환에 실패했습니다.")

        # ✅ 분기별 데이터가 있으면 추가로 표시 (자동 수집된 분기별 데이터)
        if SessionManager.is_data_available('quarterly_data'):
            st.markdown("---")
            st.subheader("📈 분기별 성과 및 추이 분석 (자동 수집)")
            st.info("💡 이 데이터는 자동 재무분석에서 수집된 분기별 데이터입니다.")
            ...
            if PLOTLY_AVAILABLE:
                ...
                st.markdown("---")
                st.subheader("📊 사용자 지정 트렌드 분석 (분기별)")
                ...
                if available_metrics:
                    ...
                    selected_companies_chart = st.multiselect(
                        ...
                        default=available_companies,
                        help="차트에 표시할 회사를 선택하세요",
                        key="manual_quarterly_chart_companies"
                    )
                    ...
                    selected_quarters = st.multiselect(
                        ...
                        default=available_quarters,
                        help="특정 분기만 선택 가능합니다",
                        key="manual_quarterly_chart_quarters"
                    )
                    ...
                    with col1:
                        bar_metrics = st.multiselect(
                            ...
                            available_metrics,
                            help="절대값 비교에 적합 (매출액, 영업이익 등)\n💡 2-3개 추천",
                            key="manual_quarterly_chart_bar_metrics"
                        )
                    ...
                    with col2:
                        line_metrics = st.multiselect(
                            ...
                            available_metrics,
                            help="트렌드 분석에 적합 (비율, 성장률 등)\n💡 2-3개 추천",
                            key="manual_quarterly_chart_line_metrics"
                        )
                    ...
                    with opt_col1:
                        chart_height = st.selectbox("차트 높이", [400, 500, 600, 700, 800], index=2, key="manual_quarterly_chart_height")
                    
                    with opt_col2:
                        show_values = st.checkbox("수치 표시", value=False, help="데이터 포인트에 값 표시", key="manual_quarterly_show_values")
                    
                    with opt_col3:
                        compact_legend = st.checkbox("범례 압축", value=True, help="범례를 더 작게 표시", key="manual_quarterly_compact_legend")
                    ...
                    if (bar_metrics or line_metrics) and not filtered_data.empty:
                        ...
                        if flexible_chart:
                            ...
                            flexible_chart.update_layout(height=chart_height)
                            st.plotly_chart(flexible_chart, use_container_width=True, key="manual_quarterly_flexible_trend")
            ...
            if not gap_analysis.empty:
                ...
                if PLOTLY_AVAILABLE:
                    ...
                    if gap_chart is not None:
                        st.plotly_chart(gap_chart, use_container_width=True, key="manual_gap_chart")
                    else:
...
def main():
    ...
    with st.sidebar:
        ...
        data_summary = {
            ...
            "수동 업로드 데이터": SessionManager.is_data_available('manual_financial_data'),
...
def render_manual_upload_tab():
    ...
    if SessionManager.is_data_available('manual_financial_data'):
        ...
        # ✅ 수동 업로드 데이터로 차트 생성 - 분기별 데이터가 없어도 표시
        st.markdown("---")
        st.subheader("📊 수동 업로드 데이터 차트 분석")
        
        # ✅ 수동 데이터를 차트용 형태로 변환
        manual_chart_data = convert_manual_data_to_quarterly_format(final_df)
        
        if not manual_chart_data.empty and PLOTLY_AVAILABLE:
            st.info("📊 수동 업로드된 재무 데이터를 차트로 분석할 수 있습니다.")
            
            # 실제 데이터에서 사용 가능한 지표들을 동적으로 확인
            all_columns = list(manual_chart_data.columns)
            exclude_cols = ['분기', '회사', '보고서구분', '연도', '분기번호']
            available_metrics = [col for col in all_columns if col not in exclude_cols]
            ...
            if available_metrics:
                # 1단계: 회사 선택
                st.markdown("**🏢 1단계: 표시할 회사 선택**")
                available_companies = list(manual_chart_data['회사'].unique()) if '회사' in manual_chart_data.columns else []
                selected_companies_chart = st.multiselect(
                    "회사를 선택하세요",
                    available_companies,
                    default=available_companies,
                ...
                # 2단계: 차트 구성 (수동 데이터는 보통 연간 데이터이므로 분기 선택 생략)
...
# ✅ 수동 업로드 재무 데이터를 차트용 분기별 형태로 변환하는 함수 추가
def convert_manual_data_to_quarterly_format(financial_data):
    """수동 업로드된 재무 데이터를 분기별 차트 형태로 변환"""
    if financial_data is None or financial_data.empty:
        return pd.DataFrame()
    ...
    try:
        ...
        if chart_rows:
            ...
            # 피벗: 각 지표를 컬럼으로 변환
            pivoted = chart_df.groupby(['분기', '회사']).first().reset_index()
            
            # 여러 지표를 하나의 행으로 합치기
            final_df = pivoted.groupby(['분기', '회사']).sum().reset_index()
        ...
        st.warning(f"수동 데이터 차트 변환 중 오류: {e}")
        return pd.DataFrame()
