...
def create_google_news_tab():
    ...
    if api_key:
        ...
        # 검색 설정 섹션
        st.markdown("### 🔍 뉴스 검색 설정")
        
        # 1. 추천 검색 쿼리 (사용자 편의를 위한 미리 정의된 쿼리)
        st.markdown("**📋 추천 검색 쿼리**")
        st.markdown("*목적: 사용자에게 '이런 키워드로 검색해보세요'를 제안합니다*")
        ...
        selected_query = st.selectbox(
            "추천 쿼리 선택",
            recommended_queries,
            help="미리 정의된 검색 쿼리 중에서 선택하세요"
        )
        
        # 2. 실제 검색 쿼리 (실제 뉴스 검색에 사용)
        st.markdown("**📑 실제 검색 쿼리**")
        st.markdown("*목적: 실제로 뉴스를 검색할 때 사용됩니다*")
        ...
        with col1:
            search_query = st.text_input(
                "검색어 입력",
                value=selected_query,  # 추천 쿼리 선택값을 기본값으로 설정
        ...
        st.markdown("**💡 사용법 안내**")
        st.markdown("""
        - **추천 쿼리만 사용**: 미리 정의된 키워드 중에서 선택
        - **직접 입력만 사용**: 자유롭게 검색어 입력
        - **둘 다 사용**: 편의성과 자유도를 모두 제공 (현재 설정)
        """)
